...
function WeatherForecast() {
    ...
    return (
        <div className="row-span-10  col-span-3 bg-white/10 backdrop-blur-2xl shadow-md rounded-md p-2 h-full hover:scale-102 transition ease-in duration-75">
            {dailyForecast.map(
                ({ date_epoch, maxtemp_c, mintemp_c, condition, code }) => (
                    <div
                        className="grid grid-cols-4 gap-0 py-2 px-2 mb-3 "
                        key={date_epoch}
                    >
                        <p className="col-span-1 pr-3 text-ellipsis overflow-hidden ">
                            {checkDay(date_epoch)}
                        </p>
                        ...
                        <span className="overflow-hidden text-nowrap text-end">
                            {Math.floor(maxtemp_c)}&deg;/{Math.floor(mintemp_c)}
                            &deg;
                        </span>
                    </div>
                )
